...
type DMLSnapshotGoHost = {
  go?: {
    app?: {
      App?: DMLSnapshotBackend;
    };
  };
};

const EMPTY_DML_SNAPSHOT_BACKEND: DMLSnapshotBackend = {};
...
export const resolveDMLSnapshotBackend = (): DMLSnapshotBackend => {
  if (typeof window === 'undefined') return EMPTY_DML_SNAPSHOT_BACKEND;
  return (window as Window & DMLSnapshotGoHost).go?.app?.App || EMPTY_DML_SNAPSHOT_BACKEND;
};
