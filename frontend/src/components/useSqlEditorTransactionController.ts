...
export const useSqlEditorTransactionController = ({
  ...
  const requestManagedCommitConfirmation = useCallback(async (
    transaction: PendingSqlEditorTransaction,
    connection: SavedConnection,
    source: FinishSqlEditorTransactionSource,
  ): Promise<boolean> => {
    ...
    return true;
  }, [downgradeToManualTransaction, translateMessage]);
  ...
  const finishPendingSqlTransaction = useCallback(async (
    ...
    if (action === 'commit') {
      ...
      // 连接查不到时 fail-closed。不能放进 confirmProductionRisk 走它的
      ...
      if (requiresProductionRiskConfirmation(connection)) {
        const approved = await requestManagedCommitConfirmation(transaction, connection, source);
        if (!approved) {
          return;
        }
      }
    ...
    clearAutoCommitTimer,
    rawErrorDetail,
    requestManagedCommitConfirmation,
    resolveTransactionConnection,
    translateMessage,
    updatePendingSqlTransaction,
  ]);
