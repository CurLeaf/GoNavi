...
import {
  ...
  resolveVisibleSidebarTreeForDragSort,
  setSidebarTreeSqlDragData,
} from './sidebarTreeDragSort';
...
describe('sidebar tree drag sort', () => {
  ...
  it('allows dragging connections and groups, but not folder-columns leaves', () => {
    const table = tableNode('conn-1', 'main', 'public', 'users');
    const columns: SidebarTreeNode = {
    ...
    const tree: SidebarTreeNode[] = [{
      ...
      title: 'tables',
      type: 'object-group',
      children: [{ ...table, children: [columns] }],
    }];

    expect(isSidebarTreeNodeDraggable({ type: 'connection' })).toBe(true);
    expect(isSidebarTreeNodeDraggable({ type: 'tag' })).toBe(true);
    expect(isSidebarTreeNodeDraggable(table)).toBe(true);
    expect(isSidebarTreeNodeDraggable(columns)).toBe(false);
    expect(isSidebarTreeNodeDraggable({ type: 'object-group' })).toBe(false);
    expect(canDropSidebarTreeOrderNode(tree, columns, table, -1)).toBe(false);
    expect(canDropSidebarTreeOrderNode(tree, table, columns, -1)).toBe(false);
  });

  it('computes drop targets from the visible tree after hidden object groups are filtered', () => {
    const users = tableNode('conn-1', 'main', 'public', 'users');
    const orders = tableNode('conn-1', 'main', 'public', 'orders');
    const seqFirst = objectNode('sequence', 'seq-first', 'seq_first');
    const seqSecond = objectNode('sequence', 'seq-second', 'seq_second');
    const tree: SidebarTreeNode[] = [{
      key: 'conn-1-main',
      title: 'main',
      type: 'database',
      children: [
        ...
        },
        {
          ...
          children: [seqFirst, seqSecond],
        },
      ],
    }];
    const visible = resolveVisibleSidebarTreeForDragSort(tree, ['sequences']);

    expect(visible[0].children?.map((node) => node.key)).toEqual(['tables']);
    expect(canDropSidebarTreeOrderNode(visible, seqSecond, seqFirst, -1)).toBe(false);
    expect(canDropSidebarTreeOrderNode(visible, orders, users, -1)).toBe(true);
  });
});
