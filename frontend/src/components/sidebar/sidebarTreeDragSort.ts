...
const SIDEBAR_TREE_FOLDER_LEAF_TYPES = new Set([
  'folder-columns',
  'folder-indexes',
  'folder-fks',
  'folder-triggers',
...
export const isSidebarTreeFolderLeaf = (
  node: Pick<SidebarTreeNode, 'type'> | null | undefined,
): boolean => SIDEBAR_TREE_FOLDER_LEAF_TYPES.has(String(node?.type || ''));

export const isSidebarTreeOrderNode = (
  node: Pick<SidebarTreeNode, 'type'> | null | undefined,
): boolean => (
  !isSidebarTreeFolderLeaf(node)
...
export const isSidebarTreeNodeDraggable = (
  node: Pick<SidebarTreeNode, 'type'> | null | undefined,
): boolean => isSidebarHostTreeNode(node) || isSidebarTreeOrderNode(node);

export const resolveVisibleSidebarTreeForDragSort = <T extends {
  type?: string;
...
): T[] => filterSidebarTreeByHiddenObjectGroups(nodes, hiddenObjectGroups);
...
export const canDropSidebarTreeOrderNode = (
  ...
  if (isSidebarTreeFolderLeaf(source) || isSidebarTreeFolderLeaf(target)) return false;
...
export const resolveSidebarHostTreeMove = (options: {
  ...
  if (options.dragNode.type === 'tag') {
    ...
    if (!id || isConnectionTagDescendant(id, destination.targetParentTagId, options.connectionTags)) {
      return null;
    }
...
export const resolveSidebarTreeOrderDropAtEvent = (
  ...
  if (!dragNode || !isSidebarTreeOrderNode(dragNode)) return null;
  const hit = resolveSidebarDropDomHit(event);
  if (!hit?.metrics) return null;
  const hoveredNode = findSidebarTreeNode(treeData, hit.key);
  if (!hoveredNode || !isSidebarTreeOrderNode(hoveredNode) || isSidebarTreeFolderLeaf(hoveredNode)) {
    return null;
  }
  let dropNode: SidebarTreeNode = hoveredNode;
  let resolvedHit = hit;
...
export const resolveSidebarTreeOrderDrop = (
  ...
  const targetParent = findSidebarTreeNodeParent(treeData, String(target.key));
  if (
    isSidebarTreeFolderLeaf(source)
    || isSidebarTreeFolderLeaf(target)
...
export const commitSidebarTreeOrderDrop = (options: {
  ...
  const visibleTree = options.visibleTreeData ?? options.treeData;
  if (!resolveSidebarTreeOrderDrop(
    visibleTree,
    options.dragNode,
    options.dropNode,
