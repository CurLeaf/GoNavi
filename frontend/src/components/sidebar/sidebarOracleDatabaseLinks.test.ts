...
describe('supportsOracleDatabaseLinks', () => {
  it('treats OceanBase Oracle mode as the Oracle dialect and skips Dameng', () => {
    const oceanBaseOracle = {
      id: 'conn-ob',
      name: 'OB Oracle',
      config: {
        type: 'oceanbase',
    ...
    const dameng = {
      id: 'conn-dm',
      name: 'DM',
      config: { type: 'dameng', host: '127.0.0.1', port: 5236, user: 'SYSDBA', database: 'DAMENG' },
    } as unknown as SavedConnection;

    expect(supportsOracleDatabaseLinks(oracleConnection)).toBe(true);
    expect(supportsOracleDatabaseLinks(oceanBaseOracle)).toBe(true);
    expect(supportsOracleDatabaseLinks(mysqlConnection)).toBe(false);
    expect(supportsOracleDatabaseLinks(dameng)).toBe(false);
  });
});
