...
import { Button, Input, type ModalProps } from 'antd';
...
/** 查询编辑器 SQL 片段选择弹窗，从 QueryEditor 抽出以控制该文件行数。 */
const SqlSnippetPickerModal: React.FC<SqlSnippetPickerModalProps> = ({
  ...
  <Modal
    ...
    onCancel={onClose}
    footer={null}
    styles={snippetPickerModalStyles(darkMode)}
  >
    <div
    ...
    >
      <div style={{ fontSize: 12, lineHeight: 1.6, color: mutedTextColor(darkMode) }}>
...
const mutedTextColor = (darkMode: boolean): string => (
  darkMode ? 'rgba(255,255,255,0.65)' : 'rgba(16,24,40,0.6)'
);

const snippetPickerModalStyles = (darkMode: boolean): NonNullable<ModalProps['styles']> => ({
  content: {
    borderRadius: 16,
...
interface SqlSnippetPickerListProps {
  snippets: SqlSnippet[];
  darkMode: boolean;
  emptyLabel: string;
  onInsertSnippet: (snippet: SqlSnippet) => void;
...
const SqlSnippetPickerList: React.FC<SqlSnippetPickerListProps> = ({
  snippets,
  darkMode,
  emptyLabel,
  onInsertSnippet,
...
interface SqlSnippetPickerItemProps {
  snippet: SqlSnippet;
  darkMode: boolean;
  onInsertSnippet: (snippet: SqlSnippet) => void;
}

const SqlSnippetPickerItem: React.FC<SqlSnippetPickerItemProps> = ({
  snippet,
  darkMode,
  onInsertSnippet,
}) => {
