...
vi.mock('antd', async () => {
  ...
  const omit = (props: HostProps, keys: string[]) => Object.fromEntries(
    Object.entries(props).filter(([key]) => !keys.includes(key)),
  );
  const Button = ({ children, icon, ...props }: HostProps) => ReactModule.createElement(
    'button',
    omit(props, ['size', 'type']),
    icon,
    children,
  );
  const Input = (props: HostProps) => ReactModule.createElement(
    'input',
    omit(props, ['size', 'status']),
  );
  const Select = ({ popupRender, open, ...props }: HostProps) => ReactModule.createElement(
    'div',
  ...
  const Tooltip = ({ children }: HostProps) => ReactModule.createElement(ReactModule.Fragment, null, children);
...
const renderSelect = async (maxRows: number, onMaxRowsChange = vi.fn()) => {
  ...
  const selectHost = () => renderer.root.findAllByType(Select)[0] as unknown as SelectHost;
  const openMenu = async () => {
    await act(async () => {
      selectHost().props.onOpenChange?.(true);
    });
  };
  const closeMenu = async () => {
    await act(async () => {
      selectHost().props.onOpenChange?.(false);
    });
  };
  return { renderer, onMaxRowsChange, openMenu, closeMenu, selectHost };
};
...
describe('QueryEditorToolbarMaxRowsSelect custom rows', () => {
  it('renders the custom input only inside the dropdown and submits a typed value', async () => {
    const { renderer, onMaxRowsChange, openMenu, selectHost } = await renderSelect(5000);

    expect(renderer.root.findAllByProps({ 'data-query-editor-max-rows-input': 'true' })).toHaveLength(0);

    await openMenu();
    const select = selectHost();
    expect(select.props.popupMatchSelectWidth).toBe(false);
    expect(select.props.options?.map((option) => option.value)).toEqual([100, 500, 1000, 5000, 20000, 0]);
  ...
  it.each(['', '0', '-1', '12.5', '1e3', 'abc', '50001'])(
    ...
    async (value) => {
      ...
      expect(selectHost().props.open).toBe(true);
      renderer.unmount();
    },
  );
  ...
  it('echoes a non-enumerated value by appending it to the options', async () => {
    const { renderer, openMenu, selectHost } = await renderSelect(12345);
    await openMenu();
    const select = selectHost();
    expect(select.props.value).toBe(12345);
    expect(select.props.options?.map((option) => option.value)).toEqual([100, 500, 1000, 5000, 20000, 0, 12345]);
