...
export type TriggerDraftCommitResult =
  | { ok: true; trigger: TriggerDefinition }
  | { ok: false; reason: 'name_required' | 'duplicate'; name?: string };

export const resolveTriggerDraftCommit = (
  sql: string,
  triggers: TriggerDefinition[],
  mode: 'create' | 'edit',
  selectedName?: string,
): TriggerDraftCommitResult => {
  const parsed = parseTriggerDraftFromSql(sql);
  if (!parsed?.name) return { ok: false, reason: 'name_required' };
  const duplicate = triggers.some((item) => {
    if (mode === 'edit' && selectedName && item.name === selectedName) return false;
    return item.name.toUpperCase() === parsed.name.toUpperCase();
  });
  if (duplicate) return { ok: false, reason: 'duplicate', name: parsed.name };
  return { ok: true, trigger: parsed };
};
