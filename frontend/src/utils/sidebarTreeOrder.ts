...
const toPinPart = (value: unknown): string => {
...
export const buildSidebarTablePinKey = (
  ...
  const parts = [connectionId, dbName, schemaName, tableName].map(toPinPart);
...
export const buildSidebarDatabasePinKey = (
  ...
  const parts = [connectionId, dbName].map(toPinPart);
