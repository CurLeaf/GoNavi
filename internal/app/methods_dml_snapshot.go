...
//
// 与 ApplyChanges 的分工：那里负责把变更写进数据库，这里只在提交成功后
// 把 ChangeSet 里的 before-image 落成可还原线索。不新增 RPC 参数，旧前端零改动。
...
// 纳秒时间戳 + 进程内原子自增序列在单进程 Wails 后端下足够唯一，且无需引入依赖。
var dmlSnapshotSequence atomic.Int64

func buildDMLSnapshotID() string {
	seq := dmlSnapshotSequence.Add(1)
	return time.Now().Format("20060102150405.000000000") + "-" + strconv.FormatInt(seq, 10)
}
