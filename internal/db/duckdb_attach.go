...
func (d *DuckDB) AttachExternalDatabase(ctx context.Context, spec ExternalAttachSpec) error {
	...
	if spec.SecretName == "" {
		spec.SecretName = externalSecretName(spec.Alias)
	}
	// 第二道防线：SECRET 名拼入 DDL，必须同为安全标识符
	if !duckDBAttachAliasPattern.MatchString(spec.SecretName) {
...
// ListExternalAttachments 实现 ExternalAttachmentLister：返回本驱动创建且当前
...
func (d *DuckDB) ListExternalAttachments(ctx context.Context) ([]ExternalAttachmentInfo, error) {
	...
	for alias, spec := range d.attachments {
		info, keep, err := d.listedAttachmentLocked(ctx, alias, spec)
		if err != nil {
			return nil, err
		}
		if keep {
...
func (d *DuckDB) listedAttachmentLocked(ctx context.Context, alias string, spec duckDBAttachmentSpec) (ExternalAttachmentInfo, bool, error) {
	attached, err := d.aliasAttached(ctx, alias)
	if err != nil {
		return ExternalAttachmentInfo{}, false, err
	}
	if !attached {
		...
		return ExternalAttachmentInfo{}, false, nil
	}
...
func (d *DuckDB) attachLocked(ctx context.Context, spec ExternalAttachSpec, desired duckDBAttachmentSpec) error {
	...
	case ExternalAttachKindMySQL, ExternalAttachKindPostgres:
		return d.attachSecretBackedLocked(ctx, spec, desired)
	case ExternalAttachKindSQLite:
		if err := d.ensureExtensionLoaded(ctx, "sqlite_scanner"); err != nil {
			return err
		}
		return d.execAttachAndRemember(ctx, spec, desired)
...
func (d *DuckDB) attachSecretBackedLocked(ctx context.Context, spec ExternalAttachSpec, desired duckDBAttachmentSpec) error {
...
func (d *DuckDB) execAttachAndRemember(ctx context.Context, spec ExternalAttachSpec, desired duckDBAttachmentSpec) error {
	if err := d.execAttach(ctx, spec); err != nil {
		return err
	}
	d.attachments[spec.Alias] = desired
...
var _ ExternalAttachmentLister = (*DuckDB)(nil)
